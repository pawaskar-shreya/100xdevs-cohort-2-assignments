...
/**
    Ans: 
        ...
        - The another that we want to happen every second is the increment of counter. Hence that must also go inside the anonymous function after the callback as counter needs to be incremented after printing. 
        ...
        - Another good thing to note is that, we could have incremented counter in doThis() also, but when pass a global variable is passed to functions, its local copy is created inside the functions. The each function will have its seperate local copy. A local copy modified in one function will only be available in that function. Hence when we try to modify counter inside doThis(), it updates the local copy in that function, but it is never made availale to counterWrapper and hence everytime a 0 is printed which was sent in the very first callback call. 
